import Link from 'next/link';
import { getProductList, getProductPages } from '@/app/lib/sql';
import Search from '@/app/ui/dashboard/search';
import Pagination from '@/app/ui/dashboard/pagination';
import { Button } from '@/app/ui/shadcn/ui/button';
import ProductActions from '@/app/ui/dashboard/products/product-actions';

export const dynamic = 'force-dynamic';

export default async function Page({
  searchParams,
}: {
  searchParams?: Promise<{ query?: string; page?: string }>;
}) {
  const resolvedSearchParams = await searchParams;
  const query = resolvedSearchParams?.query || '';
  const currentPage = Number(resolvedSearchParams?.page) || 1;
  const numberOfPages = await getProductPages(query);

  const productList = await getProductList(query, currentPage);
  return (
    <>
      <div className="flex w-full items-center justify-between">
        <h1 className="text-2xl font-semibold text-foreground">Productos</h1>
      </div>
      <div className="mt-4 flex items-center justify-between gap-2 md:mt-8">
        <Search placeholder="Buscar productos..." />
        <Link href={'/dashboard/products/create'}>
          <Button>Añadir Producto</Button>
        </Link>
      </div>
      <table className="mt-5 w-full table-auto overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <thead className="h-10 bg-primary/12 text-foreground">
          <tr>
            <th className="text-sm font-bold">Nombre</th>
            <th className="text-sm font-bold">Detalle</th>
          </tr>
        </thead>
        <tbody className="text-center">
          {productList.map((product) => (
            <tr
              key={product.name}
              className="w-full border-b border-border py-1 text-sm odd:bg-accent/45 even:bg-card hover:bg-primary/10"
            >
              <td className="whitespace-nowrap px-3 py-1">{product.label}</td>
              <td className="whitespace-nowrap py-1 pl-6 pr-3">
                <ProductActions name={product.name} label={product.label} />
              </td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="mt-5 flex w-full justify-center">
        <Pagination totalPages={numberOfPages} />
      </div>
    </>
  );
}
